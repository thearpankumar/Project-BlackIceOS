...
async def login_user(
    credentials: UserLogin, request: Request, db: Session = Depends(get_database)
):
    """
    ...
    if not user or not user.is_active:
        logger.warning(
            f"Login failed - user not found or inactive: {credentials.username}"
        )
        raise HTTPException(
    ...
    try:
        ...
        return TokenResponse(
            ...
            encrypted_api_keys=encrypted_keys,  # type: ignore
        )
...
async def refresh_token(refresh_request: dict, db: Session = Depends(get_database)):
    """
...
async def change_password(
    ...
    try:
        ...
        logger.info(
            f"Password changed for user: {current_user.username} (ID: {current_user.id})"
        )
...
async def add_api_key(
    ...
    try:
        ...
        if existing_key:
            # Update existing key
            existing_key.encrypted_key = encrypted_key  # type: ignore
            existing_key.last_used = None  # type: ignore  # Reset usage timestamp
            db.commit()
...
async def delete_api_key(
    ...
    if key_name not in ["groq", "google_genai"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid API key name"
        )
...
async def get_user_stats(
    ...
    return UserStatsResponse(
        ...
        last_login=cast(datetime | None, current_user.last_login),
        account_created=cast(datetime, current_user.created_at),
