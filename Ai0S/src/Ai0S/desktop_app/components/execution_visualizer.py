...
class ExecutionVisualizer(ctk.CTkFrame):
    ...
    def __init__(self, parent, theme=None, on_step_click: Callable[[str], None] = None):
    ...
    def set_screenshot_callback(self, callback: Callable) -> None:
        """Set the screenshot callback for handling screenshot requests."""
        self.screenshot_callback = callback
    
    def reset(self) -> None:
        """Reset the execution visualizer to initial state."""
        try:
            # Clear current plan
            self.current_plan = None
            ...
            logger.info("Execution visualizer reset")
        except Exception as e:
            logger.debug(f"Error during execution visualizer reset: {e}")
