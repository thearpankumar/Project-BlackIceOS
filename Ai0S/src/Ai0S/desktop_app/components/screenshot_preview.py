...
class ScreenshotPreview(ctk.CTkFrame):
    ...
    def __init__(self, parent, theme=None, on_screenshot_click: Callable[[Dict[str, float]], None] = None):
    ...
    def update_size(self) -> None:
        """Update the size of the screenshot preview."""
        # Update preview size based on current widget size
        try:
            if hasattr(self, 'preview_size'):
                ...
                if width > 1 and height > 1:  # Valid dimensions
                    # Calculate new preview size maintaining aspect ratio
                    aspect_ratio = 16/9  # Default aspect ratio
                    if width/height > aspect_ratio:
                        new_height = height - 40  # Leave space for controls
                        new_width = int(new_height * aspect_ratio)
                    else:
                        new_width = width - 20  # Leave space for padding
                        new_height = int(new_width / aspect_ratio)
                    
                    self.preview_size = (max(new_width, 200), max(new_height, 150))
        except Exception as e:
            logger.debug(f"Error updating screenshot preview size: {e}")