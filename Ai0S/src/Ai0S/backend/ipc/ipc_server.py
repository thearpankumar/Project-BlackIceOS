...
class IPCServer:
    ...
    async def start_server(self) -> None:
        ...
        try:
            ...
            # Start WebSocket server with improved timeout settings
            ...
            try:
                async for message in websocket:
                    try:
                        ...
                        logger.warning(f"Message processing timeout for client {client_id}")
                        await self._send_error(client_id, "Message processing timeout")
                    except Exception as e:
    ...
    async def _connection_health_monitor(self) -> None:
        """Monitor connection health and detect issues early."""
            ...
            try:
                ...
                for client_id, client in self.clients.items():
                    try:
                        ...
                        # Check for very stale connections (no heartbeat in 90 seconds)
                        time_since_heartbeat = (current_time - client.last_heartbeat).total_seconds()
                        if time_since_heartbeat > 90:
                            logger.warning(f"Client {client_id} appears unresponsive (no heartbeat for {time_since_heartbeat:.1f}s)")
                            unhealthy_clients.append(client_id)
                            continue
                            
                        # Send ping to check responsiveness every 2 minutes
                        connection_duration = (current_time - client.connected_at).total_seconds()
                        if connection_duration > 120 and connection_duration % 120 < 30:
                            try:
                        ...
                        logger.debug(f"Health check error for client {client_id}: {e}")
                        unhealthy_clients.append(client_id)
                
                # Cleanup unhealthy clients
                for client_id in unhealthy_clients:
                    logger.info(f"Cleaning up unhealthy client: {client_id}")
                    await self._disconnect_client(client_id, "connection_unhealthy")
